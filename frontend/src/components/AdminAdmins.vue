  ...
  <div class="admin-admins-container">
    ...
    <header class="admin-header">
      <div class="header-content">
        <div class="header-info">
          <h1 class="header-title">Admins Management</h1>
          <p class="header-subtitle">Manage all administrator accounts</p>
        </div>
        <div class="header-actions">
          <button class="action-btn primary" @click="openAddAdminModal">
    ...
    <main class="admin-main">
      ...
      <div v-else-if="error" class="error-container">
        ...
        <h3>Unable to Load Admins</h3>
        <p>{{ error }}</p>
        <button @click="fetchAdmins" class="retry-btn">Try Again</button>
      </div>

      <!-- Admins Table -->
      <div v-else class="admins-table-section">
        <div class="section-header">
          <h2 class="section-title">Admins List</h2>
          <span class="total-count">Total: {{ filteredAdmins.length }} admins</span>
        </div>
              ...
              <tr v-for="admin in paginatedAdmins" :key="admin.id">
                <td>
                  <div class="admin-cell">
                    <img :src="admin.profileImage || defaultAdminImage" :alt="admin.firstName" class="admin-avatar">
                    <div>
                ...
                <td>
                  <div class="contact-cell">
                    ...
                    <div class="contact-email">{{ admin.email || 'No email' }}</div>
                  </div>
                </td>
                <td>
                ...
                <td>
                  <span class="join-date">{{ formatDate(admin.createdAt) }}</span>
                </td>
                <td>
                  <div class="action-buttons">
                    <button class="action-btn-icon edit" @click="editAdmin(admin)" title="Edit">
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                        <path d="M21.731 2.269a2.625 2.625 0 00-3.712 0l-1.157 1.157 3.712 3.712 1.157-1.157a2.625 2.625 0 000-3.712zM19.513 8.199l-3.712-3.712-12.15 12.15a5.25 5.25 0 00-1.32 2.214l-.8 2.685a.75.75 0 00.933.933l2.685-.8a5.25 5.25 0 002.214-1.32L19.513 8.2z" />
                      </svg>
                    </button>
                    <button class="action-btn-icon delete" @click="confirmDeleteAdmin(admin)" title="Delete">
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
        ...
        <!-- Pagination -->
        <div v-if="!isLoading && !error && filteredAdmins.length > 0" class="pagination">
          <button 
          ...
          <form @submit.prevent="saveAdmin">
            ...
            <div class="modal-footer">
              ...
              <button type="submit" class="modal-btn primary" :disabled="saving">
                ...
                <span v-else>{{ editingAdmin ? 'Update' : 'Create' }} Admin</span>
              </button>
            </div>
          </form>
        </div>
      ...
      <div class="modal-content small">
        ...
        <div class="modal-body">
          ...
          <p class="delete-message">
            ...
            <strong>{{ itemToDelete?.firstName }} {{ itemToDelete?.lastName }}</strong>?
          </p>
...
export default {
  ...
  data() {
    return {
      ...
      // Default image
      defaultAdminImage: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?ixlib=rb-4.0.3&auto=format&fit=crop&w=200&q=80'
    }
  },
  ...
  methods: {
    async fetchAdmins() {
      ...
      try {
        ...
        const response = await fetch(`${this.apiBaseUrl}/admin/users/admins/all`, {
          method: 'GET',
          headers: {
        ...
        this.error = err.message || 'Failed to load admins'
        this.admins = []
        this.filteredAdmins = []
      } finally {
    ...
    async saveAdmin() {
      ...
      try {
        ...
        if (this.editingAdmin) {
          ...
          alert('Edit functionality will be implemented when API endpoint is available')
          this.closeModal()
        } else {
          // Create new admin
          const registerPayload = {
            first_name: this.adminForm.firstName,
            last_name: this.adminForm.lastName,
          ...
          this.successDetails = `${this.adminForm.firstName} ${this.adminForm.lastName} has been added as an administrator.`
    ...
    async confirmDelete() {
      ...
      try {
        ...
        // Note: You need a delete endpoint for admins
        // This is a placeholder - adjust according to your API
        const response = await fetch(`${this.apiBaseUrl}/admin/users/${this.itemToDelete.id}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${authToken}`
          }
        ...
        this.admins = this.admins.filter(a => a.id !== this.itemToDelete.id)
        this.filteredAdmins = this.filteredAdmins.filter(a => a.id !== this.itemToDelete.id)
        
        this.successMessage = 'Admin Deleted Successfully!'
        this.successDetails = `${this.itemToDelete.firstName} ${this.itemToDelete.lastName} has been removed from the system.`
        this.showSuccessModal = true
